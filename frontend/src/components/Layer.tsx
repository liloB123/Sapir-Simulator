...
const Layer: React.FC<LayerProps> = ({name, icon, color, description}) => {
    const { getLayer: getToggle, setLayer: setToggle} = useLayer();
    const isActive = getToggle(name);

    const handleToggle = () => {
        setToggle(name, !isActive)
    };

    const Icon = icon;

    return (
        <div className={`p-3 border bg-slate-800/30 rounded-lg flex items-center justify-between h-22 gap-8 ${
                  isActive
